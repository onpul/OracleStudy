...
FROM TBL_JUMUN;
--------------------
SELECT 주문번호, 제품코드, 주문량, 주문일자
FROM 교집합한 테이블
JOIN 합집합한 테이블
ON 제품코드 AND 주문량 같을 때
--------------------
SELECT 주문번호, 제품코드, 주문량, 주문일자
FROM 
(
...
98779	맛동산	20	2022-03-02 14:09:07
98780	모옹쉘	20	2022-03-02 14:13:40
98781	빼빼로	30	2022-03-02 14:13:43
98764	고래밥	10	2022-03-02 09:30:02
98765	빼빼로	20	2022-03-02 09:30:33
98766	맛동산	30	2022-03-02 09:31:13
98767	홈런볼	40	2022-03-02 09:31:34
...
SELECT EMPNO, ENAME, JOB, SAL
FROM TBL_EMP
WHERE SAL = ALL (800, 1600, 1250, 2975, 1250, 2850, 2450, 3000, 5000, 950, 3000, 1500, 2000, 1700, 2500, 1000);
--==>> 조회 결과 없음
...
FROM
(
    SELECT EMPNO "사원번호", ENAME "사원명", JOB "직종명", SAL "급여", DEPTNO "부서"
         , DENSE_RANK()OVER(PARTITION BY DEPTNO ORDER BY SAL DESC) "순위"
    FROM TBL_EMP
) T 
WHERE T.순위 = 1
  AND T.부서 = 20;
...
-- 취업 시 데이터베이스에 대한 문제가 꽤 많이 나오니, 데이터베이스 관련해서도 잘 알아둘 것
...
UPDATE TBL_SAWON
SET SANAME = (SELECT SANAME -- 안은 쑝쑝쑝~
              FROM TBL_SAWONBACKUP
              WHERE SANO = TBL_SAWON.SANO)
WHERE SANAME = '똘똘이'; -- 밖은 웅~
--==>> 18개 행 이(가) 업데이트되었습니다.
